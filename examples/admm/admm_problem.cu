...
namespace ba {
template <typename T>
ADMMSubproblem<kGPU, T, false>::ADMMSubproblem(const ADMMOption<T> &option,
  ...
  m_shared_extrinsics_sizes[0].resize(num_ranks, 0);
  m_shared_extrinsics_sizes[1].resize(num_ranks, 0);
  m_shared_extrinsics_dicts[0].resize(num_ranks);
  m_shared_extrinsics_dicts[1].resize(num_ranks);
  m_shared_intrinsics_sizes[0].resize(num_ranks, 0);
  m_shared_intrinsics_sizes[1].resize(num_ranks, 0);
  m_shared_intrinsics_dicts[0].resize(num_ranks);
  m_shared_intrinsics_dicts[1].resize(num_ranks);
  m_shared_camera_data[0].resize(num_ranks);
  m_shared_camera_data[1].resize(num_ranks);

  m_cpu_shared_extrinsics_sizes[0].resize(num_ranks, 0);
  m_cpu_shared_extrinsics_sizes[1].resize(num_ranks, 0);
  m_cpu_shared_extrinsics_dicts[0].resize(num_ranks);
  m_cpu_shared_extrinsics_dicts[1].resize(num_ranks);
  m_cpu_shared_intrinsics_sizes[0].resize(num_ranks, 0);
  m_cpu_shared_intrinsics_sizes[1].resize(num_ranks, 0);
  m_cpu_shared_intrinsics_dicts[0].resize(num_ranks);
  m_cpu_shared_intrinsics_dicts[1].resize(num_ranks);
  m_cpu_shared_camera_data[0].resize(num_ranks);
  m_cpu_shared_camera_data[1].resize(num_ranks);
...
template <typename T> ADMMSubproblem<kGPU, T, false>::~ADMMSubproblem() {
  CHECK_CUDA(cudaSetDevice(m_device));
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::GetCost(T &cost) const {
  cost = m_cost;
  return 0;
}

template <typename T>
int ADMMSubproblem<kGPU, T, false>::GetSurrogateCost(
    std::array<T, 3> &surrogate_cost) const {
  surrogate_cost = m_surrogate_f;
  return 0;
}

template <typename T>
int ADMMSubproblem<kGPU, T, false>::GetOption(ADMMOption<T> &option) const {
  option = m_option;
  return 0;
}
...
ADMMSubproblem<kGPU, T, false>::GetExtrinsicsIds() const {
  return m_extrinsics_ids;
}

template <typename T>
const std::vector<std::unordered_map<int_t, int_t>> &
ADMMSubproblem<kGPU, T, false>::GetIntrinsicsIds() const {
  return m_intrinsics_ids;
}

template <typename T>
const std::vector<std::unordered_map<int_t, int_t>> &
ADMMSubproblem<kGPU, T, false>::GetPointIds() const {
  return m_point_ids;
}

template <typename T>
const std::vector<std::array<int_t, 2>> &
ADMMSubproblem<kGPU, T, false>::GetExtrinsicsDicts() const {
  return m_extrinsics_dicts;
}

template <typename T>
const std::vector<std::array<int_t, 2>> &
ADMMSubproblem<kGPU, T, false>::GetIntrinsicsDicts() const {
  return m_intrinsics_dicts;
}

template <typename T>
const std::vector<std::array<int_t, 2>> &
ADMMSubproblem<kGPU, T, false>::GetPointDicts() const {
  return m_point_dicts;
}

template <typename T> int_t ADMMSubproblem<kGPU, T, false>::GetDevice() const {
  return m_device;
}

template <typename T> T ADMMSubproblem<kGPU, T, false>::GetMemoryUsage() const {
  T memory = 0;
  ...
  memory += m_extrinsics_reference.size() * sizeof(T);
  memory += m_extrinsics_dual.size() * sizeof(T);

  memory += m_intrinsics_reference.size() * sizeof(T);
  memory += m_intrinsics_dual.size() * sizeof(T);

  memory += m_prev_extrinsics.size() * sizeof(T);
  memory += m_prev_intrinsics.size() * sizeof(T);
  memory += m_prev_extrinsics_reference.size() * sizeof(T);
  memory += m_prev_intrinsics_reference.size() * sizeof(T);

  memory += m_f_values.size() * sizeof(T);

  memory += m_extrinsics_reference_cnts.size() * sizeof(int_t);
  memory += m_intrinsics_reference_cnts.size() * sizeof(int_t);
  ...
  for (int_t index = 0; index < 2; index++) {
    for (int_t rank = 0; rank < m_num_ranks; rank++) {
      memory += m_shared_extrinsics_dicts[index][rank].size() * sizeof(int_t);
      memory += m_shared_intrinsics_dicts[index][rank].size() * sizeof(int_t);
      memory += m_shared_camera_data[index][rank].size() * sizeof(T);
    }
  }
...
template <typename T>
T ADMMSubproblem<kGPU, T, false>::GetCommunicationLoad() const {
  T load = 0;

  for (int_t index = 0; index < 2; index++) {
    for (int_t rank = 0; rank < m_num_ranks; rank++) {
      load += m_shared_camera_data[index][rank].size() * sizeof(T);
    }
  }
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::Setup(
  ...
  for (const auto &index : measurement_selections[0]) {
    ...
    assert(point_info[0] == m_rank);

    if (point_info[0] != m_rank) {
      LOG(ERROR) << "Incorrect point infomation." << std::endl;
      exit(-1);
    }
  ...
  std::vector<std::unordered_set<int_t>> shared_extrinsics_dict_sets(
      m_num_ranks);
  std::vector<std::unordered_set<int_t>> shared_intrinsics_dict_sets(
      m_num_ranks);
  ...
  for (const auto &index : measurement_selections[1]) {
    ...
    if (m_extrinsics_ids[extrinsics_info[0]].count(extrinsics_info[1]) == 0) {
      LOG(WARNING) << "Extrinsics has no points in the same node associated."
                   << std::endl;
      m_extrinsics_ids[extrinsics_info[0]].insert({extrinsics_info[1], -1});
    }

    if (m_intrinsics_ids[intrinsics_info[0]].count(intrinsics_info[1]) == 0) {
      LOG(WARNING) << "Intrinsics has no points in the same node associated."
                   << std::endl;
      m_intrinsics_ids[intrinsics_info[0]].insert({intrinsics_info[1], -1});
    }

    shared_extrinsics_dict_sets[point_info[0]].insert(extrinsics_info[1]);
    shared_intrinsics_dict_sets[point_info[0]].insert(intrinsics_info[1]);
  }
  ...
  assert(m_num_points == m_point_sizes[m_rank]);

  if (m_num_points != m_point_sizes[m_rank]) {
  ...
  sfm::PinnedHostVector<T> extrinsics_reference_cnts(m_num_extrinsics, 1);
  sfm::PinnedHostVector<T> intrinsics_reference_cnts(m_num_intrinsics, 1);
  for (int_t rank = 0; rank < m_num_ranks; rank++) {
    for (const auto &index : shared_extrinsics_dict_sets[rank]) {
      extrinsics_reference_cnts[index] += 1;
    }

    for (const auto &index : shared_intrinsics_dict_sets[rank]) {
      intrinsics_reference_cnts[index] += 1;
    }
  }

  m_extrinsics_reference_cnts.resize(m_num_extrinsics);
  m_intrinsics_reference_cnts.resize(m_num_intrinsics);
  cudaMemcpyAsync(m_extrinsics_reference_cnts.data().get(),
                  extrinsics_reference_cnts.data(),
                  m_num_extrinsics * sizeof(T),
                  cudaMemcpyKind::cudaMemcpyHostToDevice, m_stream);
  cudaMemcpyAsync(m_intrinsics_reference_cnts.data().get(),
                  intrinsics_reference_cnts.data(),
                  m_num_intrinsics * sizeof(T),
                  cudaMemcpyKind::cudaMemcpyHostToDevice, m_stream);
  ...
  for (int_t rank = 0; rank < m_num_ranks; rank++) {
    if (rank == m_rank) {
      assert(shared_extrinsics_dict_sets[rank].size() == 0);
      assert(shared_intrinsics_dict_sets[rank].size() == 0);

      if (shared_extrinsics_dict_sets[rank].size() != 0 ||
          shared_intrinsics_dict_sets[rank].size() != 0) {
        LOG(ERROR) << "No extrinsics or intrinsics should be sent/received "
                      "to/from rank "
    ...
    m_shared_extrinsics_sizes[0][rank] =
        shared_extrinsics_dict_sets[rank].size();
    m_shared_extrinsics_dicts[0][rank].resize(
        m_shared_extrinsics_sizes[0][rank]);
    m_cpu_shared_extrinsics_dicts[0][rank].clear();
    m_cpu_shared_extrinsics_dicts[0][rank].reserve(
        m_shared_extrinsics_sizes[0][rank]);
    m_cpu_shared_extrinsics_dicts[0][rank].insert(
        m_cpu_shared_extrinsics_dicts[0][rank].begin(),
        shared_extrinsics_dict_sets[rank].begin(),
        shared_extrinsics_dict_sets[rank].end());
    std::sort(m_cpu_shared_extrinsics_dicts[0][rank].begin(),
              m_cpu_shared_extrinsics_dicts[0][rank].end());
    cudaMemcpyAsync(m_shared_extrinsics_dicts[0][rank].data().get(),
                    m_cpu_shared_extrinsics_dicts[0][rank].data(),
                    m_shared_extrinsics_sizes[0][rank] * sizeof(int_t),
                    cudaMemcpyKind::cudaMemcpyHostToDevice, m_stream);

    m_shared_intrinsics_sizes[0][rank] =
        shared_intrinsics_dict_sets[rank].size();
    m_shared_intrinsics_dicts[0][rank].resize(
        m_shared_intrinsics_sizes[0][rank]);
    m_cpu_shared_intrinsics_dicts[0][rank].clear();
    m_cpu_shared_intrinsics_dicts[0][rank].reserve(
        m_shared_intrinsics_sizes[0][rank]);
    m_cpu_shared_intrinsics_dicts[0][rank].insert(
        m_cpu_shared_intrinsics_dicts[0][rank].begin(),
        shared_intrinsics_dict_sets[rank].begin(),
        shared_intrinsics_dict_sets[rank].end());
    std::sort(m_cpu_shared_intrinsics_dicts[0][rank].begin(),
              m_cpu_shared_intrinsics_dicts[0][rank].end());
    cudaMemcpyAsync(m_shared_intrinsics_dicts[0][rank].data().get(),
                    m_cpu_shared_intrinsics_dicts[0][rank].data(),
                    m_shared_intrinsics_sizes[0][rank] * sizeof(int_t),
                    cudaMemcpyKind::cudaMemcpyHostToDevice, m_stream);

    m_shared_camera_data[0][rank].resize(
        D_EXTRINSICS_SIZE * m_shared_extrinsics_sizes[0][rank] +
        INTRINSICS_SIZE * m_shared_intrinsics_sizes[0][rank]);
  }
  ...
  for (int_t rank = 0; rank < m_num_ranks; rank++) {
    ...
    m_shared_extrinsics_sizes[1][rank] = m_extrinsics_sizes[rank];
    m_shared_extrinsics_dicts[1][rank].resize(
        m_shared_extrinsics_sizes[1][rank]);
    m_cpu_shared_extrinsics_dicts[1][rank].resize(
        m_shared_extrinsics_sizes[1][rank]);
    std::iota(m_cpu_shared_extrinsics_dicts[1][rank].begin(),
              m_cpu_shared_extrinsics_dicts[1][rank].end(),
              m_extrinsics_offsets[rank]);
    cudaMemcpyAsync(m_shared_extrinsics_dicts[1][rank].data().get(),
                    m_cpu_shared_extrinsics_dicts[1][rank].data(),
                    m_shared_extrinsics_sizes[1][rank] * sizeof(int_t),
                    cudaMemcpyKind::cudaMemcpyHostToDevice, m_stream);

    m_shared_intrinsics_sizes[1][rank] = m_intrinsics_sizes[rank];
    m_shared_intrinsics_dicts[1][rank].resize(
        m_shared_intrinsics_sizes[1][rank]);
    m_cpu_shared_intrinsics_dicts[1][rank].resize(
        m_shared_intrinsics_sizes[1][rank]);
    std::iota(m_cpu_shared_intrinsics_dicts[1][rank].begin(),
              m_cpu_shared_intrinsics_dicts[1][rank].end(),
              m_intrinsics_offsets[rank]);
    cudaMemcpyAsync(m_shared_intrinsics_dicts[1][rank].data().get(),
                    m_cpu_shared_intrinsics_dicts[1][rank].data(),
                    m_shared_intrinsics_sizes[1][rank] * sizeof(int_t),
                    cudaMemcpyKind::cudaMemcpyHostToDevice, m_stream);

    m_shared_camera_data[1][rank].resize(
        D_EXTRINSICS_SIZE * m_shared_extrinsics_sizes[1][rank] +
        INTRINSICS_SIZE * m_shared_intrinsics_sizes[1][rank]);
  }
  ...
  m_extrinsics.resize(D_EXTRINSICS_SIZE * m_num_extrinsics);
  m_intrinsics.resize(INTRINSICS_SIZE * m_num_intrinsics);
  m_points.resize(LANDMARK_SIZE * m_num_points);

  m_extrinsics_reference.resize(D_EXTRINSICS_SIZE * m_num_extrinsics);
  m_extrinsics_dual.resize(D_EXTRINSICS_SIZE * m_num_extrinsics);
  m_extrinsics_consensus.resize(D_EXTRINSICS_SIZE * m_num_extrinsics);

  m_intrinsics_reference.resize(INTRINSICS_SIZE * m_num_intrinsics);
  m_intrinsics_dual.resize(INTRINSICS_SIZE * m_num_intrinsics);
  m_intrinsics_consensus.resize(INTRINSICS_SIZE * m_num_intrinsics);

  m_prev_extrinsics.resize(D_EXTRINSICS_SIZE * m_num_extrinsics);
  m_prev_extrinsics_reference.resize(D_EXTRINSICS_SIZE * m_num_extrinsics);

  m_prev_intrinsics.resize(INTRINSICS_SIZE * m_num_intrinsics);
  m_prev_intrinsics_reference.resize(INTRINSICS_SIZE * m_num_intrinsics);
  ...
  if (m_option.trust_region_option.max_iterations > 0 &&
    ...
    m_trust_region_extrinsics[0].resize(D_EXTRINSICS_SIZE * m_num_extrinsics);
    m_trust_region_extrinsics[1].resize(D_EXTRINSICS_SIZE * m_num_extrinsics);
    ...
    m_hess_ll.resize(D_LANDMARK_SIZE * D_LANDMARK_SIZE * m_num_points);
    m_grad_c.resize(D_CAMERA_SIZE * m_num_cameras);
    m_grad_l.resize(D_LANDMARK_SIZE * m_num_points);
    m_reduced_grad_c.resize(D_CAMERA_SIZE * m_num_cameras);

    m_hess_cc_inv.resize(D_CAMERA_SIZE * D_CAMERA_SIZE * m_num_cameras);
    m_hess_ll_inv.resize(D_LANDMARK_SIZE * D_LANDMARK_SIZE * m_num_points);

    m_pcg_x_c.resize(D_CAMERA_SIZE * m_num_cameras);
    m_pcg_x_l.resize(D_LANDMARK_SIZE * m_num_points);
    ...
    m_buffer[4].resize(std::max(
        {// buffer for gradient_n
         D_LANDMARK_SIZE * m_num_measurements,
         // buffer for preconditioned conjugate gradient method
         D_LANDMARK_SIZE * m_num_points, D_CAMERA_SIZE * m_num_cameras}));
  }
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::Accept(const Container<T> &extrinsics,
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::GetExtrinsics(
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::GetIntrinsics(
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::GetPoints(
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::Initialize(
    ...
    const std::vector<std::unordered_map<int_t, Eigen::Matrix<T, 3, 1>>>
        &point_data) {
  std::vector<Eigen::Matrix<T, 6, 1>> initial_extrinsics;
  ...
  for (const auto &index : m_extrinsics_dicts) {
    ...
    Eigen::Matrix<T, 6, 1> initial_angle_axis_extrinsics;

    const Eigen::Matrix<T, 3, 3> &rotation =
        *(const Eigen::Matrix<T, 3, 3> *)(search->second.data());
    Eigen::Matrix<T, 3, 1> &omega =
        *(Eigen::Matrix<T, 3, 1> *)(initial_angle_axis_extrinsics.data());
    sfm::math::SO3::Log(rotation, omega);
    initial_angle_axis_extrinsics.template tail<3>() = search->second.col(3);

    initial_extrinsics.push_back((initial_angle_axis_extrinsics));
  }
  ...
  cudaMemcpyAsync(m_extrinsics_reference.data().get(),
                  m_extrinsics.data().get(),
                  D_EXTRINSICS_SIZE * m_num_extrinsics * sizeof(T),
                  cudaMemcpyDeviceToDevice, m_stream);
  cudaMemset(m_extrinsics_dual.data().get(), 0,
             m_extrinsics_dual.size() * sizeof(T));
  cudaMemcpyAsync(m_extrinsics_consensus.data().get(),
                  m_extrinsics.data().get(),
                  D_EXTRINSICS_SIZE * m_num_extrinsics * sizeof(T),
                  cudaMemcpyDeviceToDevice, m_stream);

  cudaMemcpyAsync(m_intrinsics_reference.data().get(),
                  m_intrinsics.data().get(),
                  INTRINSICS_SIZE * m_num_intrinsics * sizeof(T),
                  cudaMemcpyDeviceToDevice, m_stream);
  cudaMemset(m_intrinsics_dual.data().get(), 0,
             m_intrinsics_dual.size() * sizeof(T));
  cudaMemcpyAsync(m_intrinsics_consensus.data().get(),
                  m_intrinsics.data().get(),
                  INTRINSICS_SIZE * m_num_intrinsics * sizeof(T),
                  cudaMemcpyDeviceToDevice, m_stream);

  m_extrinsics_reference_penalty = m_option.initial_extrinsics_penalty;
  m_intrinsics_reference_penalty = m_option.initial_intrinsics_penalty;
...
int ADMMSubproblem<kGPU, T, false>::Initialize(
  ...
  for (const auto &index : m_extrinsics_dicts) {
    ...
    const Eigen::Matrix<T, 3, 3> &rotation =
        *(const Eigen::Matrix<T, 3, 3> *)(extrinsics_data[index[0]][index[1]]
                                              .data());
    Eigen::Matrix<T, 3, 1> &omega =
        *(Eigen::Matrix<T, 3, 1> *)(initial_angle_axis_extrinsics.data());
    sfm::math::SO3::Log(rotation, omega);
    initial_angle_axis_extrinsics.template tail<3>() =
        extrinsics_data[index[0]][index[1]].col(3);
...
template <typename T> int ADMMSubproblem<kGPU, T, false>::Iterate() const {
  CHECK_CUDA(cudaSetDevice(m_device));

  cudaMemcpyAsync(m_prev_extrinsics.data().get(), m_extrinsics.data().get(),
                  D_EXTRINSICS_SIZE * m_num_extrinsics * sizeof(T),
                  cudaMemcpyDeviceToDevice, m_stream);

  cudaMemcpyAsync(m_prev_intrinsics.data().get(), m_intrinsics.data().get(),
                  INTRINSICS_SIZE * m_num_intrinsics * sizeof(T),
                  cudaMemcpyDeviceToDevice, m_stream);

  cudaMemcpyAsync(m_prev_extrinsics_reference.data().get(),
                  m_extrinsics_reference.data().get(),
                  D_EXTRINSICS_SIZE * m_num_extrinsics * sizeof(T),
                  cudaMemcpyDeviceToDevice, m_stream);

  cudaMemcpyAsync(m_prev_intrinsics_reference.data().get(),
                  m_intrinsics_reference.data().get(),
                  INTRINSICS_SIZE * m_num_intrinsics * sizeof(T),
                  cudaMemcpyDeviceToDevice, m_stream);

  m_trust_region_radius = std::numeric_limits<T>::max();
  m_trust_region_decreasing_ratio = 0.5;
  EvaluateSurrogateFunction(m_extrinsics, m_intrinsics, m_points, m_surrogate_f,
                            m_cost);
  Update();
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::PreCommunicate(int_t round) const {
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::PostCommunicate(int_t round) const {
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::MPICommunicate(const MPI_Comm &comm,
                                                   int_t round) const {
  int mpi_rank;
  ...
  for (int_t rank = 0; rank < m_num_ranks; rank++) {
    if (m_shared_camera_data[send_index][rank].size() !=
        D_EXTRINSICS_SIZE * m_shared_extrinsics_sizes[send_index][rank] +
            INTRINSICS_SIZE * m_shared_intrinsics_sizes[send_index][rank]) {
      LOG(ERROR) << "Inconsistent communicaiton data size." << std::endl;
      exit(-1);
    }

    if (rank == m_rank || !(m_shared_extrinsics_sizes[send_index][rank] ||
                            m_shared_intrinsics_sizes[send_index][rank])) {
      continue;
    }

    m_cpu_shared_camera_data[send_index][rank].resize(
        m_shared_camera_data[send_index][rank].size());

    cudaMemcpyAsync(
        m_cpu_shared_camera_data[send_index][rank].data(),
        m_shared_camera_data[send_index][rank].data().get(),
        m_cpu_shared_camera_data[send_index][rank].size() * sizeof(T),
  ...
  for (int_t rank = 0; rank < m_num_ranks; rank++) {
    ...
    MPI_Isend(m_cpu_shared_camera_data[send_index][rank].data(),
              m_cpu_shared_camera_data[send_index][rank].size(),
  ...
  for (int_t rank = 0; rank < m_num_ranks; rank++) {
    if (m_shared_camera_data[recv_index][rank].size() !=
        D_EXTRINSICS_SIZE * m_shared_extrinsics_sizes[recv_index][rank] +
            INTRINSICS_SIZE * m_shared_intrinsics_sizes[recv_index][rank]) {
      LOG(ERROR) << "Inconsistent communicaiton data size." << std::endl;
      exit(-1);
    }

    if (rank == m_rank || !(m_shared_extrinsics_sizes[recv_index][rank] ||
                            m_shared_intrinsics_sizes[recv_index][rank])) {
      continue;
    }

    m_cpu_shared_camera_data[recv_index][rank].resize(
        m_shared_camera_data[recv_index][rank].size());
    MPI_Irecv(m_cpu_shared_camera_data[recv_index][rank].data(),
              m_cpu_shared_camera_data[recv_index][rank].size(),
              traits<T>::MPI_FLOAT_TYPE, rank,
  ...
  for (int_t rank = 0; rank < m_num_ranks; rank++) {
    ...
    cudaMemcpyAsync(m_shared_camera_data[recv_index][rank].data().get(),
                    m_cpu_shared_camera_data[recv_index][rank].data(),
                    m_cpu_shared_camera_data[recv_index][rank].size() *
                        sizeof(T),
                    cudaMemcpyKind::cudaMemcpyHostToDevice, m_stream);
  }
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::UpdateConsensus() const {
  sfm::utils::PlusAsync(T(1.0), m_extrinsics_dual.data().get(), T(1.5),
                        m_extrinsics.data().get(),
                        m_extrinsics_dual.data().get(), m_extrinsics.size());
  sfm::utils::PlusAsync(T(1.0), m_extrinsics_dual.data().get(), T(-1.5),
                        m_extrinsics_reference.data().get(),
                        m_extrinsics_dual.data().get(), m_extrinsics.size());
  sfm::utils::PlusAsync(T(1.0), m_intrinsics_dual.data().get(), T(1.5),
                        m_intrinsics.data().get(),
                        m_intrinsics_dual.data().get(), m_intrinsics.size());
  sfm::utils::PlusAsync(T(1.0), m_intrinsics_dual.data().get(), T(-1.5),
                        m_intrinsics_reference.data().get(),
                        m_intrinsics_dual.data().get(), m_intrinsics.size());

  T extrinsics_primal_error = thrust::transform_reduce(
      thrust::cuda::par.on(m_stream),
      sfm::utils::MakeCountingIterator<int_t>(0),
      sfm::utils::MakeCountingIterator<int_t>(m_extrinsics.size()),
      [prev_extrinsics = m_prev_extrinsics.data().get(),
       prev_extrinsics_reference =
           m_prev_extrinsics_reference.data().get()] __device__(auto idx) {
        T diff = prev_extrinsics[idx] - prev_extrinsics_reference[idx];
        return diff * diff;
      },
      T(0.0), cub::Sum());

  T extrinsics_dual_error = thrust::transform_reduce(
      thrust::cuda::par.on(m_stream),
      sfm::utils::MakeCountingIterator<int_t>(0),
      sfm::utils::MakeCountingIterator<int_t>(m_extrinsics.size()),
      [curr_extrinsics_reference = m_extrinsics_reference.data().get(),
       prev_extrinsics_reference =
           m_prev_extrinsics_reference.data().get()] __device__(auto idx) {
        T diff =
            curr_extrinsics_reference[idx] - prev_extrinsics_reference[idx];
        return diff * diff;
      },
      T(0.0), cub::Sum());

  T intrinsics_primal_error = thrust::transform_reduce(
      thrust::cuda::par.on(m_stream),
      sfm::utils::MakeCountingIterator<int_t>(0),
      sfm::utils::MakeCountingIterator<int_t>(m_intrinsics.size()),
      [prev_intrinsics = m_prev_intrinsics.data().get(),
       prev_intrinsics_reference =
           m_prev_intrinsics_reference.data().get()] __device__(auto idx) {
        T diff = prev_intrinsics[idx] - prev_intrinsics_reference[idx];
        return diff * diff;
      },
      T(0.0), cub::Sum());

  T intrinsics_dual_error = thrust::transform_reduce(
      thrust::cuda::par.on(m_stream),
      sfm::utils::MakeCountingIterator<int_t>(0),
      sfm::utils::MakeCountingIterator<int_t>(m_intrinsics.size()),
      [curr_intrinsics_reference = m_intrinsics_reference.data().get(),
       prev_intrinsics_reference =
           m_prev_intrinsics_reference.data().get()] __device__(auto idx) {
        T diff =
            curr_intrinsics_reference[idx] - prev_intrinsics_reference[idx];
        return diff * diff;
      },
      T(0.0), cub::Sum());

  T extrinsics_rescale_ratio = 1.005;
  T intrinsics_rescale_ratio = 1.005;

  if (m_option.initial_extrinsics_penalty * extrinsics_primal_error >
      2.5 * extrinsics_dual_error) {
    extrinsics_rescale_ratio = m_option.increasing_penalty_ratio;
  } else if (extrinsics_dual_error > 10 * m_option.initial_extrinsics_penalty *
                                         extrinsics_primal_error) {
    extrinsics_rescale_ratio = m_option.decreasing_penalty_ratio;
  }

  if (m_option.initial_intrinsics_penalty * intrinsics_primal_error >
      2.5 * intrinsics_dual_error) {
    intrinsics_rescale_ratio = m_option.increasing_penalty_ratio;
  } else if (intrinsics_dual_error > 10 * m_option.initial_intrinsics_penalty *
                                         intrinsics_primal_error) {
    intrinsics_rescale_ratio = m_option.decreasing_penalty_ratio;
  }

  m_extrinsics_reference_penalty *= extrinsics_rescale_ratio;
  m_intrinsics_reference_penalty *= intrinsics_rescale_ratio;
  sfm::utils::ForEachAsync(
      sfm::utils::MakeCountingIterator(0), m_extrinsics.size(),
      [extrinsics_dual = m_extrinsics_dual.data().get(),
       extrinsics_penalty = m_extrinsics_reference_penalty,
       rescale_penalty_ratio = extrinsics_rescale_ratio] __device__(auto idx) {
        extrinsics_dual[idx] /= rescale_penalty_ratio;
      },
      m_stream);

  sfm::utils::ForEachAsync(
      sfm::utils::MakeCountingIterator(0), m_intrinsics.size(),
      [intrinsics_dual = m_intrinsics_dual.data().get(),
       rescale_penalty_ratio = intrinsics_rescale_ratio] __device__(auto idx) {
        intrinsics_dual[idx] /= rescale_penalty_ratio;
      },
      m_stream);
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::InitializeSurrogateFunction() {
  cudaEvent_t event;
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::AcceptAsync(
    const Container<T> &extrinsics, const Container<T> &intrinsics,
    const Container<T> &points, const std::array<T, 3> &surrogate_f,
    T cost) const {
  sfm::utils::CopyFromDictedMatrixOfArrayAsync(
      m_n_extrinsics_dicts.data().get(), D_EXTRINSICS_SIZE, 1,
  ...
  sfm::utils::CopyFromDictedMatrixOfArrayAsync(
      m_n_extrinsics_dicts.data().get(), D_EXTRINSICS_SIZE, 1,
      extrinsics.data().get(), m_extrinsics_consensus.data().get(),
      m_num_extrinsics, m_num_extrinsics, m_extrinsics_sizes[m_rank], m_stream);

  sfm::utils::CopyFromDictedMatrixOfArrayAsync(
      m_n_intrinsics_dicts.data().get(), INTRINSICS_SIZE, 1,
      intrinsics.data().get(), m_intrinsics_consensus.data().get(),
      m_num_intrinsics, m_num_intrinsics, m_intrinsics_sizes[m_rank], m_stream);
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::EvaluateObjectiveFunction(
    const Container<T> &extrinsics_data, const Container<T> &intrinsics_data,
    const Container<T> &point_data, T &cost) const {

  std::array<T, 3> surrogate_f;

  EvaluateSurrogateFunction(extrinsics_data, intrinsics_data, point_data,
                            surrogate_f, cost);
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::EvaluateSurrogateFunction(
    const Container<T> &extrinsics_data, const Container<T> &intrinsics_data,
    const Container<T> &point_data, std::array<T, 3> &surrogate_f,
    T &cost) const {
  cudaEvent_t event;
  CHECK_CUDA(cudaEventCreate(&event));
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::LevenbergMarquardtMethod() const {
  ...
  sfm::optimization::QuadraticModel<T> quadratic_model = [this]() -> void {
    this->Linearize();
  };

  sfm::optimization::RescaleDiagonal<T> rescale_diagonal =
      [this](T prev_ratio, T curr_ratio) -> void {
  ...
  sfm::optimization::Update<T> update = [this](T stepsize,
...
template <typename T> int ADMMSubproblem<kGPU, T, false>::Linearize() const {
  LinearizeAsync();
  CHECK_CUDA(cudaStreamSynchronize(m_stream));
  CHECK_CUDA(cudaGetLastError());
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::BuildLinearSystem(T ratio) const {
  cudaEvent_t event;
  CHECK_CUDA(cudaEventCreate(&event));
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::PCG(int_t &num_iters,
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::Retract(T stepsize) const {
...
template <typename T> int ADMMSubproblem<kGPU, T, false>::Update() const {
  ...
  if (m_option.trust_region_option.max_iterations > 0 &&
    ...
    sfm::utils::CopyToDictedMatrixOfArrayAsync(
        extrinsics_dicts, D_EXTRINSICS_SIZE, 1, m_extrinsics.data().get(),
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::InitializeSurrogateFunctionAsync(
  ...
  sfm::ba::EvaluateAngleAxisReprojectionLossFunctionAsync(
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::InitializeSurrogateFunctionSync(
    Container<T> &future_surrogate_f, cudaEvent_t event) {
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::RetractAsync(T stepsize) const {
  auto d_extrinsics = m_pcg_x_c.data().get();

  sfm::utils::PlusAsync(T(1.0), m_trust_region_extrinsics[0].data().get(),
                        stepsize, m_pcg_x_c.data().get(),
                        m_trust_region_extrinsics[1].data().get(),
                        D_EXTRINSICS_SIZE * m_num_cameras, m_stream);
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::LinearizeAsync() const {
  ...
  sfm::ba::LinearizeAngleAxisReprojectionLossFunctionAsync(
  ...
  sfm::utils::ForEachAsync(
      sfm::utils::MakeCountingIterator(0), m_extrinsics.size(),
      [grad = grad_c, extrinsics,
       extrinsics_reference = m_extrinsics_reference.data().get(),
       extrinsics_dual = m_extrinsics_dual.data().get(),
       rho = m_extrinsics_reference_penalty,
       num_extrinsics = m_num_extrinsics] __device__(int_t index) {
        grad[index] += rho * (extrinsics[index] - extrinsics_reference[index] +
                              extrinsics_dual[index]);
      },
      m_stream);

  sfm::utils::ForEachAsync(
      sfm::utils::MakeCountingIterator(0), m_intrinsics.size(),
      [grad = grad_c + m_extrinsics.size(), intrinsics,
       intrinsics_reference = m_intrinsics_reference.data().get(),
       intrinsics_dual = m_intrinsics_dual.data().get(),
       rho = m_intrinsics_reference_penalty,
       num_intrinsics = m_num_intrinsics] __device__(int_t index) {
        grad[index] += rho * (intrinsics[index] - intrinsics_reference[index] +
                              intrinsics_dual[index]);
      },
      m_stream);

  for (int_t n = 0; n < D_EXTRINSICS_SIZE; n++) {
    sfm::utils::ForEachAsync(
        sfm::utils::MakeCountingIterator(0), m_num_cameras,
        [hess = hess_cc + n * (D_CAMERA_SIZE + 1) * m_num_cameras,
         rho = m_extrinsics_reference_penalty] __device__(auto idx) {
          hess[idx] += rho;
        },
        m_stream);
  }

  for (int_t n = D_EXTRINSICS_SIZE; n < D_CAMERA_SIZE; n++) {
    sfm::utils::ForEachAsync(
        sfm::utils::MakeCountingIterator(0), m_num_cameras,
        [hess = hess_cc + n * (D_CAMERA_SIZE + 1) * m_num_cameras,
         rho = m_intrinsics_reference_penalty] __device__(auto idx) {
          hess[idx] += rho;
        },
        m_stream);
  }

  sfm::utils::SetSymmetricMatrixAsync(T(1.001), T(1e-5), hess_cc, D_CAMERA_SIZE,
                                      num_cameras, m_stream);

  sfm::utils::SetSymmetricMatrixAsync(T(1.001), T(1e-5), hess_ll, LANDMARK_SIZE,
                                      num_points, m_stream);
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::BuildLinearSystemAsync(
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::BuildLinearSystemSync(
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::EvaluateSurrogateFunctionAsync(
    const Container<T> &extrinsics_data, const Container<T> &intrinsics_data,
    const Container<T> &point_data, Container<T> &future_surrogate_f,
    cudaEvent_t event) const {
  assert(extrinsics_data.size() == D_EXTRINSICS_SIZE * m_num_extrinsics);
  assert(intrinsics_data.size() == INTRINSICS_SIZE * m_num_intrinsics);
  assert(point_data.size() == LANDMARK_SIZE * m_num_points);

  if (extrinsics_data.size() != D_EXTRINSICS_SIZE * m_num_extrinsics) {
  ...
  sfm::utils::TransformReduceAsync(
      reduce_buffer, reduce_buffer_size, sfm::utils::MakeCountingIterator(0),
      extrinsics_data.size(),
      [extrinsics, extrinsics_reference = m_extrinsics_reference.data().get(),
       extrinsics_dual =
           m_extrinsics_dual.data().get()] __device__(int_t index) -> T {
        T error = extrinsics[index] - extrinsics_reference[index] +
                  extrinsics_dual[index];
        return error * error;
      },
  ...
  sfm::utils::TransformReduceAsync(
      reduce_buffer, reduce_buffer_size, sfm::utils::MakeCountingIterator(0),
      intrinsics_data.size(),
      [intrinsics, intrinsics_reference = m_intrinsics_reference.data().get(),
       intrinsics_dual =
           m_intrinsics_dual.data().get()] __device__(int_t index) -> T {
        T error = intrinsics[index] - intrinsics_reference[index] +
                  intrinsics_dual[index];
        return error * error;
      },
      m_future_surrogate_f.data().get() + 2, T(0.0), cub::Sum(), m_stream);
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::EvaluateSurrogateFunctionSync(
  ...
  surrogate_f[1] *= 0.5 * m_extrinsics_reference_penalty;
  surrogate_f[2] *= 0.5 * m_intrinsics_reference_penalty;

  cost = surrogate_f[0] + surrogate_f[1] + surrogate_f[2];

  return 0;
}

template <typename T>
int ADMMSubproblem<kGPU, T, false>::
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::PreCommunicateAsync(int_t round) const {
  const int_t index = round == 0 ? 1 : 0;
  const T *extrinsics_src = round == 1 ? m_extrinsics_reference.data().get()
                                       : m_extrinsics.data().get();
  const T *intrinsics_src = round == 1 ? m_intrinsics_reference.data().get()
                                       : m_intrinsics.data().get();

  for (int_t neighbor = 0; neighbor < m_num_ranks; neighbor++) {
    if (neighbor == m_rank || !(m_shared_extrinsics_sizes[index][neighbor] ||
                                m_shared_intrinsics_sizes[index][neighbor])) {
      continue;
    }

    T *dst = m_shared_camera_data[index][neighbor].data().get();
    sfm::utils::CopyToDictedMatrixOfArrayAsync(
        m_shared_extrinsics_dicts[index][neighbor].data().get(),
        D_EXTRINSICS_SIZE, 1, extrinsics_src, dst, m_num_extrinsics,
        m_shared_extrinsics_sizes[index][neighbor],
        m_shared_extrinsics_sizes[index][neighbor], m_comm_streams[neighbor]);

    dst += D_EXTRINSICS_SIZE * m_shared_extrinsics_sizes[index][neighbor];
    sfm::utils::CopyToDictedMatrixOfArrayAsync(
        m_shared_intrinsics_dicts[index][neighbor].data().get(),
        INTRINSICS_SIZE, 1, intrinsics_src, dst, m_num_intrinsics,
        m_shared_intrinsics_sizes[index][neighbor],
        m_shared_intrinsics_sizes[index][neighbor], m_comm_streams[neighbor]);
  }
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::PreCommunicateSync(int_t round) const {
  const int_t index = round == 0 ? 1 : 0;
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::PostCommunicateAsync(int_t round) const {
  const int_t index = round == 0 ? 0 : 1;
  T *extrinsics_dst = round != 2 ? m_extrinsics_reference.data().get()
                                 : m_extrinsics_consensus.data().get();
  T *intrinsics_dst = round != 2 ? m_intrinsics_reference.data().get()
                                 : m_intrinsics_consensus.data().get();

  if (round == 0) {
    sfm::utils::CopyFromDictedMatrixOfArrayAsync(
        m_n_extrinsics_dicts.data().get(), D_EXTRINSICS_SIZE, 1,
        round == 0 ? m_extrinsics.data().get() : m_extrinsics_dual.data().get(),
        extrinsics_dst, m_num_extrinsics, m_num_extrinsics,
        m_extrinsics_sizes[m_rank], m_stream);

    sfm::utils::CopyFromDictedMatrixOfArrayAsync(
        m_n_intrinsics_dicts.data().get(), INTRINSICS_SIZE, 1,
        round == 0 ? m_intrinsics.data().get() : m_intrinsics_dual.data().get(),
        intrinsics_dst, m_num_intrinsics, m_num_intrinsics,
        m_intrinsics_sizes[m_rank], m_stream);
    ...
    for (int_t neighbor = 0; neighbor < m_num_ranks; neighbor++) {
      ...
      const T *src = m_shared_camera_data[index][neighbor].data().get();
      sfm::utils::AddFromDictedMatrixOfArrayAsync(
          m_shared_extrinsics_dicts[index][neighbor].data().get(),
          D_EXTRINSICS_SIZE, 1, src, extrinsics_dst,
          m_shared_extrinsics_sizes[index][neighbor], m_num_extrinsics,
          m_shared_extrinsics_sizes[index][neighbor], m_stream);

      src += D_EXTRINSICS_SIZE * m_shared_extrinsics_sizes[index][neighbor];
      sfm::utils::AddFromDictedMatrixOfArrayAsync(
          m_shared_intrinsics_dicts[index][neighbor].data().get(),
          INTRINSICS_SIZE, 1, src, intrinsics_dst,
          m_shared_intrinsics_sizes[index][neighbor], m_num_intrinsics,
          m_shared_intrinsics_sizes[index][neighbor], m_stream);
    }

    thrust::divides<T> op;

    auto extrinsics_reference_cnts = m_extrinsics_reference_cnts.data().get();
    for (int k = 0; k < D_EXTRINSICS_SIZE; k++) {
      thrust::transform(thrust::cuda::par.on(m_stream), extrinsics_dst,
                        extrinsics_dst + m_num_extrinsics,
                        extrinsics_reference_cnts, extrinsics_dst, op);
      extrinsics_dst += m_num_extrinsics;
    }

    auto intrinsics_reference_cnts = m_intrinsics_reference_cnts.data().get();
    for (int k = 0; k < INTRINSICS_SIZE; k++) {
      thrust::transform(thrust::cuda::par.on(m_stream), intrinsics_dst,
                        intrinsics_dst + m_num_intrinsics,
                        intrinsics_reference_cnts, intrinsics_dst, op);
      intrinsics_dst += m_num_intrinsics;
    }
  } else {
    for (int_t neighbor = 0; neighbor < m_num_ranks; neighbor++) {
...
template <typename T>
int ADMMSubproblem<kGPU, T, false>::PostCommunicateSync(int_t round) const {
  CHECK_CUDA(cudaStreamSynchronize(m_stream));
  CHECK_CUDA(cudaGetLastError());
...
template class ADMMSubproblem<kGPU, float, false>;
template class ADMMSubproblem<kGPU, double, false>;
} // namespace ba
} // namespace sfm
